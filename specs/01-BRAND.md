...
- **One icon.** Every `Button` carries a 16px icon. A button that goes somewhere or moves forward ends with `IconArrowRight`, a step back starts with `IconArrowLeft` ("Back", "Back to home"), "Load more projects" ends with `IconArrowDown`, and "Try again" starts with `IconRefresh`. An `IconButton` carries only its glyph.
