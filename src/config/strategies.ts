...
export const STRATEGIES: Record<StrategyKey, StrategyDef> = {
  'self-consumption': {
    ...
    icon: 'Sun',
    bestFor: 'Flat or tiered tariffs, where you use your own solar',
    contactSentence:
  ...
  'battery-arbitrage': {
    ...
    icon: 'Battery',
    bestFor: 'Time-of-Use tariffs, where you buy low and use high',
    contactSentence:
  ...
  'backup-resilience': {
    ...
    icon: 'Zap',
    bestFor: 'Uptime-critical sites where load-shedding hurts',
    contactSentence:
    ...
    tab: {
      ...
      body: 'Traditional backup power costs you in fuel, maintenance, servicing, noise and emissions, and in keeping generators ready for the next outage. Solar and battery storage provide automatic, uninterrupted backup power for critical operations, reducing generator runtime, lowering operating costs and improving business resilience during outages and load-shedding.',
      bulletsLabel: 'Ideal for',
      bullets: [
...
export function strategyTabs(): TabItem[] {
  return STRATEGY_ORDER.map((k) => {
    ...
    return {
      ...
      ...s.tab,
      chartKey: s.key,
      // The C&I label (src/config/ctas.ts); the link prefills a sentence naming this strategy.
      cta: { label: SERVICE_CTA['ci-solar-storage'].label, href: `/contact?intent=client&strategy=${s.key}` },
    };
  });
}
...
/**
 * The sentence that prefills the contact form's message. `fromFinder` is true
 * only when the visitor arrived from the Strategy Finder's result; a strategy
 * tab's button says the visitor is interested, not that they used the Finder.
 */
export function contactMessageForStrategy(key: string, fromFinder = false): string | undefined {
  const s = (STRATEGIES as Record<string, StrategyDef>)[key];
  if (!s) return undefined;
  return fromFinder
    ? s.contactSentence
    : `I'm interested in ${s.label} for my business and would like to book a discovery meeting.`;
}
