...
/* ------------------------------------------------------------------ *
 *  PLACEHOLDER STATS — swap these for the real numbers you'll provide
 * ------------------------------------------------------------------ */
const STATS = [
  { value: '10 MWp + 8 MWh', label: 'Installed & under development' },
  { value: '6',              label: 'EPC partners' },
  { value: '40+',            label: 'Projects completed' },
  { value: '10',             label: 'Projects under development' },
] as const;

/* Location blobs — coloured with the 6 solution accent colours, no letters.
   Gauteng cluster nudged east (right). Easy to fine-tune. */
const MARKERS = [
...
function SouthAfricaMap({ fill }: { fill: string }) {
  return (
    <svg viewBox="0 0 1024 1024" className="w-full h-full" role="img" aria-label="Phoenix Energy national footprint across South Africa">
...
function FootprintBlock({ mapFill }: { mapFill: string }) {
      ...
      <div className="relative w-full max-w-[460px] mx-auto aspect-square">
        ...
        {MARKERS.map((m, i) => (
          <motion.div
            ...
            animate={inView ? { opacity: 1, scale: 1 } : {}}
            transition={{ type: 'spring', stiffness: 320, damping: 18, delay: 0.6 + i * 0.12 }}
          ...
          >
            ...
            <span
              ...
              style={{
                ...
                height: 18,
                background: m.color,
                boxShadow: `0 3px 8px ${m.color}80`,
              }}
            />
          </motion.div>
        ))}
...
function Intro({ centered = false }: { centered?: boolean }) {
  return (
    <div className={centered ? 'max-w-2xl mx-auto text-center mb-14' : 'mb-14 max-w-xl'}>
      ...
      <p className="font-body text-sm leading-[1.8] text-[#6B7280]">
        ...
        that designs, finances, acquires and optimises
...
export default function AfricaPreviewPage() {
  return (
    <div className="bg-[#F5F5F5] min-h-screen">
      ...
      <div className="page-container py-8 border-b border-[#E5E7EB]">
        ...
        <h1 className="font-display font-extrabold text-2xl text-[#1A1A1A]">Africa Footprint Section</h1>
        <p className="font-body text-sm text-[#6B7280] mt-1">
          Replaces the dark 2×2 stat grid in AboutStory. Stats are <strong>placeholders</strong> — send me the real numbers.
        </p>
      ...
      {/* ── Chosen direction: centered intro + stats/map, white bg ─ */}
      <section className="bg-white py-16 md:py-24">
        <div className="page-container">
          <Intro centered />
          <FootprintBlock mapFill="rgba(57,87,92,0.16)" />
        </div>
      </section>
